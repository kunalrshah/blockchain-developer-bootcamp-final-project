...
import { addBuy } from "../../../store/actions";
import {
  loadWeb3,
  loadAccount,
  loadPearlZZExchange
...
function Buy(props) {
  ...
  const onSubmit = async () => {
    if (name.trim() === "") {
      ...
      console.log("pearlzzExchange::buy.js", tempExchange);
            console.log(tempdata.issuer, accountID.toString(), 'admin', tempdata.points)
            tempExchange.methods.buyPoints(tempdata.issuer, accountID.toString(), 'admin', tempdata.points)
            .send({ from: accounttemp }).on('transactionHash', (hash) => {
              dispatch(addBuy(tempdata));
            })
  ...
  return (
    <>
      ...
      <button className="btn btn-secondary col-sm-6 mb-3" onClick={onSubmit}>Confirm Buy</button>
    </>
  );
}
